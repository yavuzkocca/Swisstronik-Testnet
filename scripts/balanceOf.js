...
async function main() {
    ...
    const replace_contractAddress = "0x4Fb616FA8B44779F8e98D3A779A1898B5b67712F";

    // Get the signer (your account)
    const [signer] = await hre.ethers.getSigners();

    // Create a contract instance
    const replace_contractFactory = await hre.ethers.getContractFactory("YVZKC");
    const contract = replace_contractFactory.attach(replace_contractAddress);

    // Send a shielded query to retrieve balance data from the contract
    const replace_functionName = "balanceOf";
    const replace_functionArgs = ["0xAd7020180d32a80811CD9A70414c79F8A09bB39a"];
